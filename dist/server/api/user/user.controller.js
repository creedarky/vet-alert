...
function index(req, res) {
  return _sqldb.User.findAll({
    attributes: ['id', 'nombre', 'apellido', 'email', 'provider']
  }).then(function (users) {
...
function destroy(req, res) {
  return _sqldb.User.destroy({ where: { id: req.params.id } }).then(function () {
    res.status(204).end();
  }).catch(handleError(res));
}
...
function me(req, res, next) {
  ...
  return _sqldb.User.find({
    ...
    attributes: ['id', 'nombre', 'apellido', 'email', 'provider'],
    include: [{
      model: _sqldb.Rol, as: 'rol',
      include: [_sqldb.Permiso]
    }]
  }).then(function (user) {
