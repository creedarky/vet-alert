...
function index(req, res) {
  return _sqldb.Rol.findAll().then(respondWithResult(res)).catch(handleError(res));
}

// Gets a single Rol from the DB
function show(req, res) {
  return _sqldb.Rol.find({
    where: {
      _id: req.params.id
    }
...
function create(req, res) {
  return _sqldb.Rol.create(req.body).then(respondWithResult(res, 201)).catch(handleError(res));
}
...
function upsert(req, res) {
  ...
  return _sqldb.Rol.upsert(req.body, {
    where: {
      _id: req.params.id
    }
